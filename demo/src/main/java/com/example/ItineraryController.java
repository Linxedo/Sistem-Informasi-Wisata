...
public class ItineraryController {
    ...
    @FXML private TextField itineraryNameField;
    @FXML private DatePicker startDatePicker;
    @FXML private DatePicker endDatePicker;
    @FXML private TextArea notesTextArea;
    @FXML private TableView<ItineraryDetail> detailTable;
    @FXML private TableColumn<ItineraryDetail, Integer> urutanColumn;
    @FXML private TableColumn<ItineraryDetail, String> namaDestinasiColumn;
    @FXML private TableColumn<ItineraryDetail, String> hargaColumn;
    @FXML private TableColumn<ItineraryDetail, LocalDate> tanggalColumn;
    ...
    private void setupTableColumns() {
        urutanColumn.setCellValueFactory(cellData -> 
            new javafx.beans.property.SimpleIntegerProperty(cellData.getValue().getUrutan()).asObject());
        namaDestinasiColumn.setCellValueFactory(cellData -> 
            new javafx.beans.property.SimpleStringProperty(cellData.getValue().getNamaDestinasi()));
        hargaColumn.setCellValueFactory(cellData -> 
            new javafx.beans.property.SimpleStringProperty(cellData.getValue().getHargaFormatted()));
        tanggalColumn.setCellValueFactory(cellData -> 
            new javafx.beans.property.SimpleObjectProperty<>(cellData.getValue().getTanggalKunjungan()));
    ...
    private void loadItinerariesFromDatabase() {
        ...
        try {
            ...
            LOGGER.log(Level.INFO, "Loaded " + allItineraries.size() + " itineraries for user: " + currentUser.getUsername());
            
        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, "Error loading itineraries from database", e);
            showError("Error", "Gagal memuat itinerary dari database");
        } finally {
            try { if (rs != null) rs.close(); } catch (Exception e) { LOGGER.log(Level.WARNING, "Error closing ResultSet", e); }
            try { if (pstmt != null) pstmt.close(); } catch (Exception e) { LOGGER.log(Level.WARNING, "Error closing PreparedStatement", e); }
            try { if (conn != null) conn.close(); } catch (Exception e) { LOGGER.log(Level.WARNING, "Error closing Connection", e); }
        }
    }
    ...
    private void loadItineraryDetails(int itineraryId) {
        ...
        try {
            ...
            javafx.application.Platform.runLater(() -> {
                totalBiayaLabel.setText("Total Biaya: Rp " + String.format("%,d", totalCost).replace(",", "."));
            });
    ...
    private void insertNewItinerary(String namaRencana, LocalDate startDate, LocalDate endDate) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                currentItinerary = new Itinerary(itineraryId, currentUser.getId(), namaRencana, 0, startDate, endDate, notesTextArea.getText(), "Draft");
    ...
    private void addDestinationToItinerary() {
        if (currentItinerary == null || selectedDestinasi == null) return;
    ...
    private void displayItineraryDetails(Itinerary itinerary) {
        ...
        if (itinerary.getTanggalMulai() != null && itinerary.getTanggalSelesai() != null) {
            long duration = java.time.temporal.ChronoUnit.DAYS.between(itinerary.getTanggalMulai(), itinerary.getTanggalSelesai()) + 1;
